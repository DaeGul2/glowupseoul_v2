...
적용 결과:
```
devices:            11   (Ulthera·Shurink·Thermage·InMode·Botox·Juvederm 등)
procedure_devices:  11   (각 device 의 hero_procedure_slug 가 primary 링크로)
mechanisms:         31   (기존)
procedures:         30   (기존)
```

`/api/catalog/bootstrap` 응답에 `devices` + `procedure_devices` 컬렉션 + counts 정상 노출.
`/api/catalog/devices/ulthera` detail endpoint 도 `procedures: 1 → hifu_face` 반환.
