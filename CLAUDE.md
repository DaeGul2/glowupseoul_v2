...
### 즉시 (다음 세션 시작 시)
1. **Admin 한국어화 + 필드 설명 풀세트** ← 진행 중 (`§24`)
   - 모든 컬럼에 `label_ko` + `help_ko` 추가 (specs.js)
   - 모델 이름 한국어 (브랜드/병원/시술/…)
   - group 이름 한국어 (식별/사진/언어 응대/…)
   - 사이드바·대시보드·리스트·편집·파트너·로그인 UI 전부 한국어
   - 필드명 raw (slug/contract_status 등) 노출 X → 친화적 한글 라벨 + 작은 회색 hint
2. **catalog fetch 전환** — `client/src/data/db.js` 의 procedures/hospitals/concerns/categories/devices 를 `/api/catalog/*` 호출로 교체. React Query 도입 결정. (`§19-7`, `§21-8`)
3. **procedures.category_id NULL 일괄 채우기** — mock 의 `category_slug` 누락. UPDATE 쿼리 또는 mock 재시드. (`§20`, `§21-8`)
4. **OG / favicon 이미지 드롭** — `client/public/og-cover.jpg`, `favicon.svg`, `apple-touch-icon.png` (`§22-8`)
